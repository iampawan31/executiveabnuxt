export const faqs = [
  {
    title: 'COVID-19',
    faqs: [
      {
        title:
          'What is EAB doing for added safety during the COVID-19 pandemic?',
        content:
          '<p>EAB has evolved our safety standards to ensure that our clients can enjoy vehicle shopping while mitigating the risk of COVID-19 exposure.</p><br/><p>Any vehicle that is purchased through our service has undergone additional mandatory safety precautions before you receive it. These precautions include the sanitization of the entire interior of the vehicle both before and after any inspections/reconditioning work has taken place, and after any person enters the vehicle before you take possession of it.</p><br/><p>Additionally, all EAB personnel and vendors are being held to guidelines above locally mandated ordinance to protect the health of our workforce and all of our current and future clients.<br />We are continually monitoring the evolution of the COVID-19 pandemic and are diligently at work optimizing our processes to ensure that you enjoy an exceptional, and safe vehicle buying experience.</p>',
      },
    ],
  },
  {
    title: 'GENERAL',
    faqs: [
      {
        title: 'How long does it take to complete a purchase using EAB?',
        content:
          'Purchases usually take 5 days to complete once you have chosen the vehicle you want. However, it is possible for you to receive your vehicle within 24 hours if requested and needed.',
      },
      {
        title: 'How is EAB different than Truecar?',
        content:
          'Buying services like Truecar still require you to go into the dealership and do not actually facilitate your vehicle sale for you. Instead of connecting you to a dealership we secure the vehicle you want directly from the manufacturer at factory direct pricing, handle all paperwork on your behalf, and have the vehicle shipped directly to you, free of charge. ',
      },
      {
        title: 'How is EAB different than Carvana and Shift?',
        content:
          '<p>Carvana and Shift function as online dealership and sell you vehicles that they have in stock, operating just like a traditional car dealer but online with vehicle delivery.</p><br/><p>EAB, unlike all dealerships, does not stock inventory. EAB operates by facilitating vehicle sales directly with manufacturers and their entire nationwide inventory. This gives you a buying advantage as you are not limited to selecting from just what is currently in the inventory of dealers and online dealers.</p>',
      },
    ],
  },
  {
    title: 'NEW VEHICLE',
    faqs: [
      {
        title: 'Can you provide financing on a new car?',
        content:
          'Yes, you can utilize the manufacturers financing and any incentives they have.',
      },
      {
        title: 'How do I obtain manufacturer financing?',
        content:
          'EAB will provide you with a consumer financing application for you to fill out. We will then submit it to the manufacturer on your behalf.',
      },
      {
        title:
          'Can I get the same manufacturer incentives offered by automakers using EAB?',
        content:
          'Yes. However, under most circumstances our pricing beats the ongoing consumer incentives and discounts.',
      },
      {
        title:
          'Can EAB obtain a car I have already began negotiating with at a dealership?',
        content:
          'No. Unfortunately once you have contacted a dealership in person your information has been registered to their system. EAB will be unable to secure a discount on your behalf because of this.',
      },
      {
        title:
          'Can EAB obtain a car a dealer or other website has listed for sale?',
        content:
          'No. EAB only obtains vehicles directly from manufacturers and cannot help facilitate a vehicle listing through a dealer.',
      },
      {
        title: 'Do I have to go to a dealership to pick up my car?',
        content:
          'No. EAB will have the vehicle delivered to your location of your choosing free of cost.',
      },
    ],
  },
  {
    title: 'PRE-OWNED VEHICLES',
    faqs: [
      {
        title: 'What’s the oldest vehicle EAB can purchase?',
        content:
          'EAB can provide any aged vehicle as long as it is able to pass our strict inspection process.',
      },
      {
        title: 'Does EAB provide vehicle history for pre-owned cars?',
        content:
          'Yes. We utilize CARFAX, Autocheck and NMVTIS. Any pre-owned vehicle we sell must have a clean history with 0 accidents - no exceptions.',
      },
      {
        title: 'Is EAB Pricing always the cheapest for pre-owned vehicles?',
        content:
          '<p>EAB’s pricing will always provide you with the best value for your money. However, this does not mean that our pricing will be the “cheapest” you will find for a specific make and model vehicle online.</p><br/><p>This is because we do not compromise on the quality of the vehicles we select. We will only procure and sell vehicles that are accident free, have a clean maintenance record, and that will be able to pass our extremely strict vehicle inspection and reconditioning process.</p><br/><p>We have not, and will never, compromise on the quality of a vehicle in order to save a client money.</p>',
      },
    ],
  },
  {
    title: 'WARRANTY',
    faqs: [
      {
        title:
          'Does a pre-owned vehicle purchased through EAB have a warranty?',
        content:
          'Yes, if the vehicle is still within the manufacturer’s warranty period it will remain with the vehicle. However, if the vehicle is already outside of manufacture warranty you will need to purchase an extended warranty.',
      },
      {
        title: 'Can I buy an extended warranty through EAB?',
        content:
          'Yes, we are able to offer the same extended warranties offered by the manufacturer at a discounted rate.',
      },
      {
        title:
          'What’s covered under an extended warranty purchased through EAB?',
        content:
          'EABs extended warranties are extensions of the manufacturers original warranty, so the same coverage that was available when the car was new will be available through your extended warranty.',
      },
    ],
  },
]
